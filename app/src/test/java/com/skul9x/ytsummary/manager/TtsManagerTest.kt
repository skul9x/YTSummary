...
class TtsManagerTest {
    ...
    @Test
    fun `test speakInChunks enqueues multiple items for long text`() {
        val longText = "A".repeat(4000) // Will be split into 2 chunks if MAX_CHUNK_SIZE is 3500
        
        // Use reflection to call private speakInChunks
        val method = TtsManager::class.java.getDeclaredMethod("speakInChunks", String::class.java, Boolean::class.java)
        method.isAccessible = true
        method.invoke(ttsManager, longText, true)
        
        // Verify tts.speak was called twice
        verify(exactly = 2) { mockTts.speak(any(), any(), null, any()) }
    }
}
