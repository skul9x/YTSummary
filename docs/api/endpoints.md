...
| Google REST | `v1beta/models/[MODEL]:generateContent` | Sent directly from client side wrapper | OkHttp POST, x-goog-api-key Authentication Header | Sends request payload including SafetySettings and GenerationConfig with system prompts tailored specifically for Android App. Receives `AiResult.Success` or HTTP Error variants back to App Context. |
